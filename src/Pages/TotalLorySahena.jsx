...
const TotalLorySahena = () => {
    ...
    const fetchLories = async () => {
        try {
            const res = await axios.get('https://api.ashrafulenterprise.com/trips/all-lories-sahena');
    ...
    const handleAddLory = async () => {
        ...
        try {
            const res = await axios.post('https://api.ashrafulenterprise.com/trips/add-lory-sahena', {
    ...
    const handleDeleteLory = async (number) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await axios.delete(`https://api.ashrafulenterprise.com/trips/delete-lory-sahena/${encodeURIComponent(number)}`);
                    if (res.data.success) {
    ...
    const handleViewDocuments = async (number) => {
        ...
        setLoading(true);
        try {
            const docRes = await axios.get(`https://api.ashrafulenterprise.com/trips/lory-details-sahena/${number}`);
            ...
            <div className="relative overflow-hidden bg-gradient-to-r from-blue-700 to-indigo-800 p-8 rounded-[2rem] shadow-2xl mb-10">
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="text-center md:text-left">
                        ...
                        <p className="text-blue-100 mt-2 font-medium opacity-90">Sahena Enterprise Management System</p>
                    </div>
                ...
                {loryList.map((number, index) => (
                    <div key={index} className="group relative bg-white border border-slate-200 rounded-[1.5rem] p-6 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 overflow-hidden">
                        <div className="relative z-10">
                            ...
                            <p className="text-slate-400 text-xs font-bold uppercase mb-6">Sahena Enterprise</p>
