...
const LoryWorkHistoryImam = () => {
    ...
    const fetchAllHistory = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://fatema-naz-server-6.onrender.com/all-lory-works-imam'); 
  ...
  const handleRowClick = async (id) => {
    ...
    if (confirmDelete) {
        try {
            const response = await fetch(`https://fatema-naz-server-6.onrender.com/delete-lory-work-imam/${id}`, {
                method: 'DELETE',
            });
