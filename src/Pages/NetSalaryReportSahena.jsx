...
const NetSalaryReportSahena = () => {
    ...
    const fetchNetSalaryData = async () => {
        try {
            setHasSearched(true);
            ...
            setTrips([]);
            alert("নিট স্যালারি ডাটা লোড করা সম্ভব হয়নি।");
        }
    };

    // ডাটা ফিল্টার ও প্রসেসিং (Net Salary = driverTotalReceive)
    const salaryRows = (trips || []).flatMap(trip => 
        (trip.rows || []).map(row => ({
            ...row,
            date: trip.date,
            dipoName: trip.dipoName,
            netSalary: Number(row.driverTotalReceive) || 0 
        }))
    ).filter(row => {
        const hasSalary = row.netSalary > 0;
    ...
    const totalNetAmount = salaryRows.reduce((sum, row) => sum + row.netSalary, 0);
    ...
    const downloadPDF = () => {
        ...
        doc.text("Sahena Enterprise - Net Salary Report", 40, 45);
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex justify-between items-center mb-8 no-print">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-3xl font-extrabold text-teal-900">Net Salary Report</h1>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-teal-100 mb-8 no-print">
                    ...
                    <button onClick={fetchNetSalaryData} className="btn bg-teal-700 hover:bg-teal-800 text-white w-full mt-6 font-bold rounded-xl border-none shadow-lg shadow-teal-100 transition-all">Generate Net Report</button>
                </div>
                                        ...
                                        <tbody>
                                            {salaryRows.length > 0 ? salaryRows.map((row, idx) => (
                                                <tr key={idx} className="hover:bg-teal-50/50 transition-colors border-b border-teal-50">
                                                    ...
                                                    <td className="text-teal-700 font-black text-lg">{row.netSalary.toLocaleString()}</td>
                                                </tr>
                                            )) : (
...
export default NetSalaryReportSahena;