...
const LoryWorkHistoryDiba = () => {
    ...
    const fetchAllHistory = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://fatema-naz-server-6.onrender.com/all-lory-works-diba'); 
  ...
  const handleRowClick = async (id) => {
    ...
    if (confirmDelete) {
        try {
            const response = await fetch(`https://fatema-naz-server-6.onrender.com/delete-lory-work-diba/${id}`, {
                method: 'DELETE',
            });
