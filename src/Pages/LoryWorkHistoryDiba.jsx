...
const LoryWorkHistoryDiba = () => {
    ...
    const fetchAllHistory = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://api.ashrafulenterprise.com/all-lory-works-diba'); 
    ...
    const handleRowClick = async (id) => {
        ...
        if (confirmDelete) {
            try {
                const response = await fetch(`https://api.ashrafulenterprise.com/delete-lory-work-diba/${id}`, {
                    method: 'DELETE',
                });
                const data = await response.json();
                ...
                <div className="bg-gradient-to-r from-slate-900 to-slate-800 p-8 rounded-[2.5rem] shadow-xl mb-8 text-white relative overflow-hidden">
                    <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                        <div>
                            ...
                            <p className="text-slate-400 mt-2 font-medium uppercase tracking-widest text-xs">Diba Ratri Fleet Records</p>
                        </div>
...
export default LoryWorkHistoryDiba;