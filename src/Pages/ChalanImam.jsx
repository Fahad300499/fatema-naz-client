...
const ChalanImam = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`https://fatema-naz-server-7.onrender.com/chalans-imam/${chalanDate}`);
    ...
    const saveToDB = async (updatedEntries) => {
        try {
            await axios.post('https://fatema-naz-server-7.onrender.com/chalans-imam', {
                date: chalanDate,
                companyName,
