...
const LoryWorkHistory = () => {
    ...
    const fetchAllHistory = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://fatema-naz-server-6.onrender.com/all-lory-works'); 
  ...
  const handleRowClick = async (id) => {
    ...
    if (confirmDelete) {
        try {
            const response = await fetch(`https://fatema-naz-server-6.onrender.com/delete-lory-work/${id}`, {
                method: 'DELETE',
            });
