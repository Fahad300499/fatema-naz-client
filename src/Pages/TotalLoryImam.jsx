...
const TotalLoryImam = () => {
    ...
    const handleViewDetails = async (number) => {
        ...
        try {
            ...
            const tripRes = await axios.get(`https://fatema-naz-server-7.onrender.com/trips-imam/${number}`);
            setRecentTrips(tripRes.data);

            // ২. লরী ডকুমেন্টস ডাটা ফেচ করা (যাতে আপডেট হওয়া ডাটা দেখা যায়)
            const docRes = await axios.get(`https://fatema-naz-server-7.onrender.com/lory-details-imam/${number}`);
