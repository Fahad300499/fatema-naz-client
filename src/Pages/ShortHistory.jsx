...
const ShortHistory = () => {
    ...
    const fetchShortData = async () => {
        try {
            ...
            const url = `https://api.ashrafulenterprise.com/short-calculations?${params.toString()}`;
    ...
    const handleDeleteRow = async (sheetId, rowId) => {
        ...
        try {
            const res = await fetch(`https://api.ashrafulenterprise.com/short-calculations/${sheetId}/row/${rowId}`, {
                method: 'DELETE',
            });
    ...
    const downloadPDF = () => {
        ...
        doc.setFontSize(10);
        doc.text("Fatema Naz Petroleum", 40, 55);
