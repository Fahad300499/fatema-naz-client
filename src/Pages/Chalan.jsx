...
const Chalan = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`https://fatema-naz-server-5.onrender.com/chalans/${chalanDate}`);
    ...
    const saveToDB = async (updatedEntries) => {
        try {
            await axios.post('https://fatema-naz-server-5.onrender.com/chalans', {
                date: chalanDate,
                companyName,
